...
csv_file_path = pathlib.Path(f'{args.output_path}/final_results.csv')
csv_file_path.parent.mkdir(parents=True, exist_ok=True)
fieldnames = ["user_prompt", "adv_suffix", "gen_str"]
...
for idx, behavior_config in enumerate(behavior_configs[14:]):
    print(f"\n=== Processing prompt {idx+1} / {len(behavior_configs)} ===")
    user_prompt = behavior_config['behaviour']
    num_steps = behavior_config['step']
    adv_string_init = behavior_config['adv_init_suffix']
    batch_size = behavior_config['batch_size']
    ...
    for i in tqdm(range(num_steps)):
        ...
        if log_entry is not None:
            row = {
                ...
                "gen_str": log_entry.get("gen_str")
            }

        if i % 10 == 0:
            submission_json_file = pathlib.Path(
                f'{args.output_path}/submission/result_{args.id}.json')
            ...
            log_json_file = pathlib.Path(
                f'{args.output_path}/log/result_{args.id}.json')
